...
public class Robot extends LoggedRobot {
  ...
  private Swerve getDrivebase(Mode mode) {
    switch (mode) {
      case REAL -> {
        return new Swerve(new SwerveIOSparkMax());
      }
      case SIM -> {
        return new Swerve(new SwerveIOSim());
      }
      default -> {
        return new Swerve(new SwerveIO() {});
      }
    }
  }
  ...
  private Superstructure getSuperstructure(Mode mode) {
    switch (mode) {
      ...
      case SIM -> {
        return new Superstructure(
            new PivotIOSim(), new IntakeIOSim(), new FeederIOSim(), new FlywheelIOSim());
      }
      default -> {
        return new Superstructure(
            new PivotIO() {}, new IntakeIO() {}, new FeederIO() {}, new FlywheelIO() {});
      }
    }
  }
